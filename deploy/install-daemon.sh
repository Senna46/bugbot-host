#!/bin/sh
# Installs pr-shadow as a user LaunchAgent (runs on login, restarts on failure).
...
PLIST_NAME="com.senna.pr-shadow"
LAUNCH_AGENTS="${HOME}/Library/LaunchAgents"
PLIST_DEST="${LAUNCH_AGENTS}/${PLIST_NAME}.plist"
LOG_DIR="${HOME}/.pr-shadow/logs"
...
sed -e "s|__PROJECT_ROOT__|$SAFE_PROJECT_ROOT|g" -e "s|__HOME__|$SAFE_HOME|g" -e "s|__NODE_PATH__|$SAFE_NODE_BIN|g" \
  "$SCRIPT_DIR/pr-shadow-daemon.plist" > "$PLIST_DEST"
chmod 644 "$PLIST_DEST"

launchctl unload "$PLIST_DEST" 2>/dev/null || true
launchctl load "$PLIST_DEST"
echo "pr-shadow daemon installed. Logs: $LOG_DIR/stdout.log and $LOG_DIR/stderr.log"
echo "Commands: launchctl list | grep pr-shadow | start/stop: launchctl start/stop $PLIST_NAME"
